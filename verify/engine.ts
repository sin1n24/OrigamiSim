...
// frame some models are decoded in.
const half = 0.5;
 ...
 * directly against the native square's own corners (e.g. yakko-san's blintz,
 * which folds corner-to-center) must NOT be piped through this. */
...
export function report(label: string, layers: L[]): void {
  console.log(`after ${label}: ${layers.length} layers`);
  layers.forEach((l, i) => console.log(`  [${i}] area=${area(l.poly).toFixed(4)}`, dedupe(toRoot(l))));
}
